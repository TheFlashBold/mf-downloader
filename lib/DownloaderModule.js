...
class DownloaderModule extends Module {
    ...
    async postInit() {
        this.telegramModule = this.app.getModule('telegram');
        if (!this.telegramModule) {
            return;
        }
        const bot = this.telegramModule.getBot();
        if (bot) {
        ...
        if (/\/search .*/.test(text)) {
            ...
            downloader.onStatus.push(async (text, forceNew) => {
                if (forceNew || !message_id) {
                    message_id = (await this.telegramModule.sendMessage(chat_id, text)).message_id;
                    if (forceNew) {
                        message_id = null;
                    }
                } else {
                    await this.telegramModule.editMessage(chat_id, message_id, text);
                }
            });
    ...
    prepareDownload(pipelineName, query) {
        ...
        const pipeline = new pipelineClass(pipelineConfig, this.app);
